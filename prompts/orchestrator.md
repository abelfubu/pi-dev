...
6. **Run the human gate with `tuicr_review`.** Only after the two-axis review passes, call `tuicr_review` with `action: open`, the pinned `repoDir: REPO_DIR`, and `baseRef: BASE_REF`. The tool resolves an immutable merge-base-to-HEAD range, opens it in a focused full-screen Herdr pane, and returns a `reviewId` plus the pinned HEAD SHA. Ask the user to review that exact diff.
   - When the user finishes, call `tuicr_review` with `action: comments` and the returned `reviewId`. An empty comment list, closing tuicr, or silence is not approval; the only passing signal is explicit user approval.
   - Close the owned review pane with `tuicr_review` action `close`. The tool rejects unknown panes, so cleanup cannot target a pre-existing pane.
   - If the user leaves comments, close the review, delegate fixes to fresh coder slices, rerun focused checks and both review axes, then open a fresh `tuicr_review`. Any code change invalidates prior approval.
