...
2. **Plan slices before delegating.** Break the work into slices. A slice covers **one behavior, one subsystem, one verification goal**, touches ≤5–8 files, and carries ≤3 acceptance criteria (split triggers, not targets). Cross-cutting work is sequential: scout/map → one implementation slice → focused verification → next slice.
   ...
   **Human plan gate — mandatory for non-trivial or ambiguous work:** before delegating any writer, synthesize the implementation plan yourself from the task, repository evidence, and scout findings. Scouts provide facts and options; the orchestrator owns the final plan.
   - Write the plan to a temporary Markdown file outside every repository. Include expected behavior, proposed architecture/modules, likely files, slice/PR graph, focused checks, non-goals, risks/assumptions, and breaking changes with migration paths.
   ...
   - Call `herdr_start` once with `command: "nvim <absolute-plan-path>"`, a clear plan-review `label`, `focus: true`, and `zoomed: true`. Ask the user to inspect the file and return with comments or explicit approval. Do not manually discover Herdr commands, inspect the pane, or close it; the user manages it.
   - If the user comments, revise the same plan file and ask for review again. Silence or leaving Neovim is not approval. Do not launch an implementation writer until the user explicitly approves the plan.
