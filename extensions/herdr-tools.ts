...
const SubagentDoneParams = Type.Object({
	parent_pane_id: Type.String({
		description: "Herdr pane ID of the parent session",
	}),
	result_file: Type.String({
		description: "Absolute path to the result file the subagent wrote",
	}),
...
async function executeSubagent(
	...
	try {
		...
		if (!profile) {
			return {
				content: [
					textContent(`Unknown profile: ${params.profile}. Available: ${Object.keys(SUBAGENT_PROFILES).join(", ")}.`),
				],
				isError: true,
		...
		if (missing.length > 0) {
			return {
				...
				isError: true,
				details: { missing } as SubagentDetails,
			};
		}

		let skillPath: string | undefined;
		if (profile.skill) {
			skillPath = resolveSkillPath(profile.skill);
			if (!existsSync(join(skillPath, "SKILL.md"))) {
				return {
					content: [
						textContent(`Skill not found: ${profile.skill}`),
					],
					isError: true,
					details: {} as SubagentDetails,
				};
		...
		if (!parentPaneId) {
			return {
				content: [
					textContent("Not running inside a Herdr-managed pane."),
				],
				isError: true,
				details: {} as SubagentDetails,
			};
		...
		if (skillPath) piArgs.push("--skill", skillPath);
		...
		return {
			...
			details: { profile: profile.name, pane: container.paneId, tab: container.tabId, resultFile } as SubagentDetails,
		};
	} catch (err) {
	...
	pi.registerTool({
		...
		async execute(_id, params) {
			...
			await notifyPane(
				...
				`subagent done: ${params.result_file} (${summary})`,
			);
			return {
				content: [
					textContent(`Notified parent pane ${params.parent_pane_id}.`),
				],
				details: {},
			};
		},
